...
export class Daemon {
  ...
  private async ingestOtel(calls: OtelCall[]): Promise<void> {
    ...
    for (const call of calls) {
      ...
      // path: a `beagle search` hit is proof the string was SENT. The messages
      // are exactly the self-reported outbound surface (prompt + tool inputs,
      // or a tool result the agent will echo back) — call.response.text is the
      // model's answer and is deliberately NOT indexed, or a sentence only ever
      // generated by the model would be reported as sent. The answer still
      // reaches the UI through the summary and the stored response body.
      let searchText = call.request.messages?.map((m) => m.content).join("\n") ?? "";
